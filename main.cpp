...
void menu_start(int rows, int cols, player *pl, bool *is_playing_game) {
    ...
    while (!is_complete && (action = getch()) != 27) {
        ...
        mvwprintw(stdscr, start_game.y - 10, start_game.x + 8, "БЕГУУЩИЙ К ЗАЧЕТУ");
...
void end_game(int rows, int cols, player *pl) {
    coord restart_game;
    const char *mesg = "Вы умерли! поставьте пожалуйста 6 балов";
    bool is_complete = false;
    int action;
    pl->x = rows / 2;
    pl->y = cols / 2;
...
int main() {
    ...
    mob mob_1(10, 20, 1, 99, 99, 99); //тестовый моб
    mob mob_2(10, 20, 1, 99, 99, 99); //тестовый моб
    mob mob_3(10, 20, 1, 99, 99, 99); //тестовый моб
    mob mob_4(10, 20, 1, 99, 99, 99); //тестовый моб
    mob mob_5(10, 20, 1, 99, 99, 99); //тестовый моб
